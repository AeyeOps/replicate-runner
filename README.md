...
- **Prompt wizard:** `replicate-runner prompt wizard --profile audra` walks through missing inputs (model, LoRA, subject, etc.), renders the final prompt, prints a ready-to-run command, and can optionally execute via `--run`. Passing `--no-interactive` enforces flags-only runs, while `--base-model-only` confirms you want to drop the LoRA. When you run the wizard without a profile, it now defaults the Replicate model to `black-forest-labs/flux-dev-lora`, auto-normalizes LoRA inputs (adding the `huggingface.co/` prefix when you omit it), and warns/auto-swaps the fields if it looks like you accidentally put the base model in the LoRA prompt.
